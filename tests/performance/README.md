...
7. Load small and large GraphQL introspection endpoints into schema documents; record time to usable explorer and subjective editor responsiveness while requesting completion/hover. In Web Inspector, record any main-thread task at or above 50 ms during schema installation and query editing. The latest schema timings are also available with `performance.getEntriesByName("purr.graphql.schema.worker-round-trip").at(-1)?.duration` and `performance.getEntriesByName("purr.graphql.schema.parse").at(-1)?.duration`.
