...
- Use only the named palette tokens: `purr`, `border`, `content`, `action`, `accent`, `method`, `status`, and `mac`. Reuse `accent` for a raw palette colour outside its method or status meaning; do not repurpose `method-*` colours for generic UI.
