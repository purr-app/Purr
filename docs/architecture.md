...
Git consumers install the frontend package from a reviewed public tag/revision; npm's `prepare` lifecycle builds `dist-core` before packing it. The native dependency uses the same Git revision through Cargo. Consumer lockfiles and extension API checks own compatibility; a developer's unrelated public working checkout must not gate application startup. The in-tree consumer fixture still uses local paths for contract testing. Until version tags are published, use an exact commit plus the declared package/crate versions; never follow moving `main` in a release.
