...
Limits: 50,000 spans / 16 MiB per normalized trace, 500 spans per IPC page, up to 256 attributes per span (4 KiB strings, 128 scalar array elements), 32 memory-cache entries within a 64 MiB serialized budget and a 60-second TTL, four concurrent operations, 15-second timeout. Cache keys bind workspace, integration, provider/config and current credential fingerprints; credentials are re-resolved before cache hits. Search checks normalized span ID, service, operation, status, attribute keys and scalar/array values in Rust, retaining each match's ancestors. Missing parents become roots; duplicate IDs and cycles are rejected before caching. Cursors bind the exact normalized trace snapshot as well as query/config/credentials, so a changed trace cannot silently corrupt incremental loading.

The viewer is execution-centric. The request selects its provider; the response UI has no provider switcher. An optional trace ID field loads another trace through the same provider; leaving it empty uses the execution’s correlation. IPC pages are collected into one bounded snapshot before publishing the virtualized waterfall, with progress and cancellation during collection. This keeps scrollbar geometry fixed even when dragging directly to the end. Reload retains the previous complete snapshot until the new one is ready. Selection/folding are retained per trace. Native global timing bounds set the scale. The single header includes correlation, start time, duration, service count, maximum depth, and total spans; bars and duration labels have separate columns. Service colors are assigned without hash collisions for the first seven services. Hover/focus shows the operation and available SQL, route, DNS or address context. Span details open only on selection, with timing, identity and attribute sections using response code typography. Changing the document, execution or integration silently cancels stale work. Cmd/Ctrl+F uses the response find control for local span/attribute matching, navigates the full snapshot and temporarily opens ancestors of the current match. Search never reloads the trace. Explicit Cancel has its own UI state. Opening Trace starts a debounced lookup; normal response display never waits for it. There is no persisted trace cache, provider-wide trace discovery, log API, event/link inspector or background auto-refresh yet.
